        ...
        <div class="p-8">
            ...
            <div class="grid lg:grid-cols-3 gap-8 mb-8">
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100 lg:col-span-2">
                    <h3 class="text-lg font-bold text-dark mb-6 text-left">Students by Class</h3>
                    @if($studentsByClass->count() > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        @foreach($studentsByClass as $class)
                        <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 hover:border-primary/30 hover:shadow-md transition-all">
                            <p class="text-2xl font-extrabold text-primary mb-1">{{ $class->count }}</p>
                            <p class="text-sm text-gray-600 font-medium">{{ $class->class_name }}</p>
                        </div>
                        @endforeach
                    </div>
                    @else
