        ...
        <div class="p-4 md:p-8">
            ...
            <div class="mb-6 md:mb-8">
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-2">
                    ...
                    <div class="flex-1">
                        <h1 class="text-2xl md:text-4xl font-bold text-gray-900 mb-1">Edit Teacher</h1>
                        <p class="text-sm md:text-base text-gray-600">Update teacher profile and account information</p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300">
                            ...
                            <div class="grid sm:grid-cols-2 gap-5">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="first_name" value="{{ old('first_name', $teacher->user->first_name) }}" required
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="last_name" value="{{ old('last_name', $teacher->user->last_name) }}" required
                                           class="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-200 placeholder-gray-400"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="phone" value="{{ old('phone', $teacher->user->phone) }}" required
                                           class="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-200 placeholder-gray-400"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date_of_birth" value="{{ old('date_of_birth', $teacher->user->date_of_birth) }}" required
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="relative">
                                        <select name="qualification"
                                            ...
                                            <option value="{{ $value }}" {{ old('qualification', $teacher->qualification) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4">
                                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="relative">
                                        <select name="status"
                                            ...
                                            <option value="inactive" {{ old('status', $teacher->status) == 'inactive' ? 'selected' : '' }}>⏸️ Inactive</option>
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-amber-700 mt-1">
                                            @if($classes->isEmpty() && $subjects->isEmpty())
                                                No classes or subjects have been created for this school yet.
...
const existingAssignments = @json($teacher->teacherSubjects ?? []);

function addSubjectAssignment(classId = '', subjectId = '') {
    ...
    const classOptions = classes.map(c => `<option value="${c.id}" ${c.id == classId ? 'selected' : ''}>${c.name}</option>`).join('');
    const subjectOptions = subjects.map(s => `<option value="${s.id}" ${s.id == subjectId ? 'selected' : ''}>${s.name}</option>`).join('');
...
// Load existing assignments or add one default
window.addEventListener('DOMContentLoaded', function() {
    if (existingAssignments && existingAssignments.length > 0) {
        // Load existing assignments
        existingAssignments.forEach(assignment => {
            addSubjectAssignment(assignment.school_class_id, assignment.subject_id);
        });
    } else if (classes.length > 0 && subjects.length > 0) {
