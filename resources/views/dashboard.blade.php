...
<div class="flex min-h-screen bg-surface">
    {{-- ========================================
         SIDEBAR
         ======================================== --}}
    <aside class="fixed top-0 left-0 bottom-0 w-64 sidebar-gradient z-40 flex flex-col transition-transform duration-300" id="sidebar">
        {{-- Logo & Brand --}}
        ...
        <div class="p-8">
            ...
            <div class="bg-gradient-to-r from-primary to-primary-dark rounded-2xl p-8 mb-8 relative overflow-hidden">
                ...
                <div class="relative z-10 text-left">
                    <h1 class="text-2xl font-bold text-white mb-1">{{ $school->name ?? 'West African Excellence Academy' }}</h1>
                    <p class="text-white/80 italic text-sm mb-4">"{{ $school->motto ?? 'Knowledge, Character, and Excellence' }}"</p>
                    <div class="flex flex-wrap items-center gap-4 text-xs font-semibold text-white/90">
                        <span class="px-2.5 py-1 bg-white/20 rounded-md">Session: {{ $currentSession->name ?? date('Y').'/'.(date('Y')+1) }}</span>
                        <span class="px-2.5 py-1 bg-white/20 rounded-md">Term: {{ $currentTerm->name ?? 'First Term' }}</span>
                        <span class="px-2.5 py-1 bg-accent text-dark rounded-md">Country: {{ $school->country ?? 'Nigeria' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                    <div class="flex items-start justify-between mb-4">
                        <div class="w-12 h-12 {{ $card['bg'] }} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                            <svg class="w-6 h-6 text-{{ $card['color'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $card['icon'] }}"/>
                            </svg>
                        </div>
                        <span class="inline-flex items-center gap-1 text-[10px] font-bold text-{{ $card['color'] }} bg-{{ $card['color'] }}/10 px-2.5 py-1 rounded-full uppercase tracking-wider">
                            {{ $card['change'] }}
                        </span>
                    </div>
            ...
            <div class="grid lg:grid-cols-3 gap-8 mb-8">
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl p-6 border border-gray-100">
                    <div class="flex items-center justify-between mb-6">
                        <div class="text-left">
                            <h3 class="text-lg font-bold text-dark">Weekly Student Attendance</h3>
                            <p class="text-sm text-gray-400">Weekly tracking of active students</p>
                        </div>
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100">
                    <h3 class="text-lg font-bold text-dark mb-6 text-left">Administrative Controls</h3>
                    <div class="space-y-3">
                        @foreach([
                        ...
                        <button class="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-100 hover:border-{{ $action['color'] }}/30 hover:bg-{{ $action['color'] }}/5 transition-all duration-200 group text-left">
                            <div class="w-9 h-9 rounded-lg bg-{{ $action['color'] }}/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                                <svg class="w-4.5 h-4.5 text-{{ $action['color'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $action['icon'] }}"/>
                                </svg>
                            </div>
                            <span class="text-sm font-semibold text-gray-700 group-hover:text-dark transition-colors">{{ $action['label'] }}</span>
                            <svg class="w-4 h-4 text-gray-300 ml-auto group-hover:text-{{ $action['color'] }} group-hover:translate-x-1 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden">
                    <div class="flex items-center justify-between p-6 border-b border-gray-100">
                        <div class="text-left">
                            <h3 class="text-lg font-bold text-dark">Recent Enrolled Students</h3>
                            <p class="text-sm text-gray-400">Latest students admitted to class levels</p>
                        </div>
                    </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="bg-gray-50/50">
                                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-6 py-3">Student</th>
                                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-6 py-3">Class/Arm</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-4 text-left">
                                        <div class="flex items-center gap-3">
                                            <div class="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center text-xs font-bold text-primary">
                                                {{ strtoupper(substr($activity['student'], 0, 2)) }}
                                            </div>
                                            <span class="text-sm font-semibold text-dark">{{ $activity['student'] }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-600 text-left">{{ $activity['action'] }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-400 text-left">{{ $activity['date'] }}</td>
                                    <td class="px-6 py-4 text-left">
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-success/10 text-success rounded-full">
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100">
                    <h3 class="text-lg font-bold text-dark mb-6 text-left">Official Announcements</h3>
                    <div class="space-y-4">
                        ...
                        <div class="p-4 rounded-xl bg-gray-50 border border-gray-100 hover:border-primary/20 transition-all text-left">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-[10px] text-gray-400 font-semibold">{{ $announcement->announced_at->format('M d, Y') }}</span>
                            </div>
