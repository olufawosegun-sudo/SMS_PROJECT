...
@section('title', 'Add Department — ' . ($school->name ?? 'EduWest Africa'))
        ...
        <div class="p-4 md:p-6 lg:p-8 max-w-3xl">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-dark">Add New Department</h1>
                        <p class="text-gray-500">Create a department to group teachers and activities</p>
                    </div>
                </div>
            </div>

            {{-- Form --}}
            <form action="{{ route('departments.store') }}" method="POST" class="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 space-y-6">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" rows="4"
                              class="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200 placeholder-gray-400"
                              placeholder="Describe the department scope, subjects, or notes...">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="text-xs text-red-600 mt-1.5 flex items-center gap-1">
                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
