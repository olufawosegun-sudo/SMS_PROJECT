        ...
        <div class="p-4 md:p-6 lg:p-8">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-dark">Edit Student</h1>
                        ...
                        <div class="bg-white rounded-2xl p-6 border border-gray-100">
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="first_name" value="{{ old('first_name', $student->user->first_name) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="text" name="last_name" value="{{ old('last_name', $student->user->last_name) }}" required
                                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                    @error('last_name')
                                    <p class="text-xs text-danger mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="date" name="date_of_birth" value="{{ old('date_of_birth', $student->user->dob ? $student->user->dob->format('Y-m-d') : '') }}" required
                                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                    @error('date_of_birth')
                                    <p class="text-xs text-danger mt-1">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="bg-white rounded-2xl p-6 border border-gray-100">
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="arm_id" id="arm_id" data-selected="{{ old('arm_id', $student->arm_id) }}"
                                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                        <option value="">Select Arm</option>
                                        @foreach($arms as $arm)
                                        <option value="{{ $arm->id }}" data-class-id="{{ $arm->class_id }}" {{ old('arm_id', $student->arm_id) == $arm->id ? 'selected' : '' }}>
                                            {{ $arm->name }}
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="admission_date" value="{{ old('admission_date', $student->admission_date ? $student->admission_date->format('Y-m-d') : date('Y-m-d')) }}"
                                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" readonly>
                            ...
                            <div class="text-center">
                                <div class="w-32 h-32 mx-auto mb-4 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden" id="photoPreview">
                                    @if($student->user->profile_photo)
                                        <img src="{{ asset('storage/' . $student->user->profile_photo) }}" class="w-full h-full object-cover">
                                    @else
                        ...
                        <div class="bg-white rounded-2xl p-6 border border-gray-100">
                            ...
                            <div class="mt-4 p-4 bg-blue-50 rounded-lg">
                                <p class="text-xs text-blue-700">
                                    ...
                                    <strong>Admission Date:</strong> {{ $student->admission_date->format('M d, Y') }}
                                </p>
                            </div>
                        </div>
                    </div>
