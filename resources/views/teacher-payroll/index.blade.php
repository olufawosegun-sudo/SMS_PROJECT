    ...
    <main class="flex-1 ml-0 lg:ml-64 transition-all duration-300">
        ...
        <div class="p-4 md:p-6 lg:p-8">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
                <div>
                    <h1 class="text-3xl font-bold text-dark mb-2">Teacher Payroll</h1>
                    <p class="text-gray-500">Record staff monthly salaries, allowances, and payroll deductions</p>
                </div>
            </div>
            @if(session('success'))
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100 h-fit">
                    ...
                    <form method="POST" action="{{ route('teacher-payroll.store') }}" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-sm font-semibold text-gray-600 mb-2">Select Teacher</label>
                            <select name="teacher_id" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm">
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th class="text-left px-6 py-4 text-xs font-bold text-gray-500 uppercase">Teacher</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm font-semibold text-dark">
                                        <p>{{ $pay->teacher->user->first_name ?? 'N/A' }} {{ $pay->teacher->user->last_name ?? '' }}</p>
                                        <p class="text-[10px] text-gray-400">Paid: {{ $pay->paid_at?->format('M d, Y') ?? '—' }}</p>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <form method="POST" action="{{ route('teacher-payroll.destroy', $pay->id) }}" onsubmit="return confirm('Remove payroll record?')">
                                            @csrf @method('DELETE')
