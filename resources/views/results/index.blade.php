        ...
        <div class="p-4 md:p-6 lg:p-8">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p class="text-gray-500">Record and review CA test and examination scores</p>
                </div>
            </div>
            @if(session('success'))
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100 h-fit">
                    <h3 class="text-lg font-bold text-dark mb-4">Record Score</h3>
                    <form method="POST" action="{{ route('results.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="class_id" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm">
                        ...
                        <div>
                            ...
                            <select name="subject_id" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm">
                        ...
                        <div>
                            ...
                            <input type="number" name="student_id" required placeholder="Enter student record ID" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm">
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-semibold text-gray-600 mb-2">CA Score (30%)</label>
                                <input type="number" name="ca_score" required min="0" max="30" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-600 mb-2">Exam Score (70%)</label>
                                <input type="number" name="exam_score" required min="0" max="70" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm">
                            </div>
                        </div>
                        <button type="submit" class="w-full py-3 bg-primary text-white rounded-xl hover:bg-primary-dark transition-colors font-semibold text-sm">Save Score</button>
                    </form>
                </div>

                {{-- Right: Results List --}}
                <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100"><h3 class="text-lg font-bold text-dark">Grades Sheet</h3></div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th class="text-left px-6 py-4 text-xs font-bold text-gray-500 uppercase">CA</th>
                                    <th class="text-left px-6 py-4 text-xs font-bold text-gray-500 uppercase">Exam</th>
                                    <th class="text-left px-6 py-4 text-xs font-bold text-gray-500 uppercase">Total</th>
                                    <th class="text-left px-6 py-4 text-xs font-bold text-gray-500 uppercase">Grade</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-4 text-sm font-semibold text-dark">{{ $r->student->user->first_name ?? 'N/A' }} {{ $r->student->user->last_name ?? '' }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $r->ca_score }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $r->exam_score }}</td>
                                    <td class="px-6 py-4 text-sm font-bold text-primary">{{ $r->total_score }}</td>
                                    <td class="px-6 py-4"><span class="px-2 py-0.5 text-xs font-bold rounded bg-accent/10 text-accent-dark">{{ $r->grade }}</span></td>
                                    <td class="px-6 py-4">
                                        <form method="POST" action="{{ route('results.destroy', $r->id) }}" onsubmit="return confirm('Delete record?')">
                                            @csrf @method('DELETE')
                                ...
                                @empty
                                <tr><td colspan="8" class="px-6 py-12 text-center text-gray-400">No score records found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
