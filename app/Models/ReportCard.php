...
class ReportCard extends Model {
    ...
    {
        if ($this->average >= 70) return 'A - Excellent';
        if ($this->average >= 60) return 'B - Very Good';
        if ($this->average >= 50) return 'C - Good';
        if ($this->average >= 40) return 'D - Fair';
        return 'F - Poor';
    }

    public function getPositionSuffix()
    {
        if (!$this->overall_position) return '';
