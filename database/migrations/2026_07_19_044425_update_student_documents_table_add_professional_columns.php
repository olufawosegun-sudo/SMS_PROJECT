...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('student_documents', function (Blueprint $table) {
            ...
            $indexExists = collect(DB::select("SHOW INDEX FROM student_documents WHERE Key_name = 'student_documents_school_id_student_id_index'"))->isNotEmpty();
            if (!$indexExists) {
                $table->index(['school_id', 'student_id']);
            }
            
            $indexExists = collect(DB::select("SHOW INDEX FROM student_documents WHERE Key_name = 'student_documents_document_type_index'"))->isNotEmpty();
            if (!$indexExists) {
                $table->index('document_type');
            }
            
            $indexExists = collect(DB::select("SHOW INDEX FROM student_documents WHERE Key_name = 'student_documents_status_index'"))->isNotEmpty();
            if (!$indexExists) {
                $table->index('status');
            }
        });
